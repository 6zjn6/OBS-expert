...
from chatgpt_rag import chat_with_gpt_new_noimage_wrapper, chat_with_gpt_new_bothimage_wrapper, warm_up_cache, \
    get_cache_stats, search_radical_explanation, search_character_by_radical, search_variant_characters, search_character_by_modern_character
import argparse
...
def validate_radical_prediction(predicted_radicals, actual_radicals):
    """
    验证预测结果，优先使用实际部首
    
    Args:
        predicted_radicals: PrototypeClassifier预测的部首列表
        actual_radicals: 从文件名解析的实际部首列表
    
    Returns:
        list: 验证后的部首列表
    """
    if not predicted_radicals:
        return actual_radicals if actual_radicals else []
    
    if not actual_radicals:
        return predicted_radicals[:1] if predicted_radicals else []
    
    # 检查实际部首是否在预测列表中
    for actual_radical in actual_radicals:
        if actual_radical in predicted_radicals:
            print(f"    ✅ 验证通过: 实际部首 '{actual_radical}' 在预测列表中")
            return [actual_radical]
    
    # 如果实际部首不在预测列表中，检查预测的部首是否合理
    print(f"    ⚠️  验证警告: 实际部首 {actual_radicals} 不在预测列表 {predicted_radicals[:3]} 中")
    
    # 优先使用实际部首，即使预测不匹配
    print(f"    🔧 使用实际部首: {actual_radicals[0]}")
    return [actual_radicals[0]]
...
def process_all_characters_generation_module():
    """处理所有字符的Generation Module评估"""
    ...
    # CSV文件路径
    generation_module_file = f'{output_dir}/final_output_generate_module.csv'

    # 创建CSV文件并写入表头
    with open(generation_module_file, mode='w', newline='', encoding='utf-8-sig') as file:
    ...
    def get_processed_characters():
        ...
        # 检查generation module文件
        if os.path.exists(generation_module_file):
            try:
                generation_df = pd.read_csv(generation_module_file)
                if len(generation_df) > 0:
                    processed_chars.update(generation_df['Character'].tolist())
                    print(f"📋 从generation module文件找到 {len(generation_df)} 条已处理记录")
            except Exception as e:
                print(f"⚠️  读取generation module文件失败: {e}")
    ...
    def get_processed_images_for_character(character):
        ...
        if os.path.exists(generation_module_file):
            try:
                generation_df = pd.read_csv(generation_module_file)
                if len(generation_df) > 0:
                    char_records = generation_df[generation_df['Character'] == character]
    ...
    for i, zi in enumerate(remaining_chars):
        ...
        for file in image_files:
            ...
            for i, radical_image in enumerate(radical_images):
                ...
                # 验证预测结果，优先使用实际部首
                actual_radical = [target_radical[i]] if i < len(target_radical) else []
                validated_radical = validate_radical_prediction(possible_radical, actual_radical)
                validated_radicals.extend(validated_radical)

            print(f"  预测的部首: {possible_radicals}")
            print(f"  实际部首: {target_radical}")
            print(f"  验证后的部首: {validated_radicals}")
            
            # 使用验证后的部首作为最终结果
            best_radicals = validated_radicals
            ...
            while retry_count < max_retries:
                try:
                    print(f"  🔄 调用Generation Module... (尝试 {retry_count + 1}/{max_retries})")
                    # 将数据库信息作为上下文传递给LLM
                    print(f"  📝 数据库信息已获取，长度: {len(database_output)} 字符")
                    print(f"  📝 数据库信息预览: {database_output[:100]}...")
                    
                    model_output_generate_module, used_baseline_fallback = chat_with_gpt_new_bothimage_wrapper(anonymous_main_path,
                                                                                                               anonymous_radical_paths,
                                                                                                               best_radicals)
                    break  # 成功，跳出重试循环
                except Exception as e:
            ...
            # 解析模型输出
            print(f"  📝 Generation Module原始输出: {model_output_generate_module[:100]}...")
            pred_type_generate, pred_reasoning_generate = parse_llm_output(model_output_generate_module)

            print(f"  Generation Module预测: {pred_type_generate} - {pred_reasoning_generate}")
            ...
            # 计算character_type准确度
            type_correct_generate = 1 if pred_type_generate == gt_type else 0

            print(f"  Generation Module类型准确: {type_correct_generate}")

            # 保存generation module结果
            with open(generation_module_file, mode='a', newline='', encoding='utf-8-sig', errors='ignore') as file:
                writer = csv.writer(file)
                writer.writerow(
                    [zi, gt_type, gt_reasoning, pred_type_generate, pred_reasoning_generate, type_correct_generate])
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='甲骨文解释系统 - Generation Module Pipeline')
    ...
    if not os.path.exists(img_zi_root):
        print(f"data/img_zi 目录不存在: {img_zi_root}")
        exit(1)

    # 直接处理所有字符
    print('🚀 开始处理所有字符 (Generation Module Pipeline)...')
    process_all_characters_generation_module()
    ...
    # 计算最终结果
    output_dir = 'output'
    generation_file = f'{output_dir}/final_output_generate_module.csv'

    if os.path.exists(generation_file):
        generation_df = pd.read_csv(generation_file)
        generation_accuracy = generation_df['Type_Correct'].mean()
        print(f"📊 Generation Module Character Type准确度: {generation_accuracy:.4f} ({generation_accuracy * 100:.2f}%)")
        print(f"📊 Generation Module总样本数: {len(generation_df)}")

    # 输出详细统计信息
    if os.path.exists(generation_file):
        print("\n📈 按字符类型统计准确度:")
        for char_type in ['象形字', '会意字', '形声字']:
            generation_type_acc = generation_df[generation_df['Ground_Truth_Type'] == char_type]['Type_Correct'].mean()
            generation_count = len(generation_df[generation_df['Ground_Truth_Type'] == char_type])

            print(f"{char_type}:")
            print(f"  Generation Module: {generation_type_acc:.4f} ({generation_count}个样本)")

    print("🎉 Generation Module结果已保存到output目录")
